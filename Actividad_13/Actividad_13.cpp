//Nelson Alexander Evora Diaz 00066819 (codigo hecho y compilado en Dev-C++; trabajando en parejas con Carmen Elisa Lopez Alvarado)
#include <iostream>
#include <string>
using namespace std;
...
void eliminarMisterio(){
    int M=misterio;
    Nodo *p = pInicio, *q = NULL, *r;
 
    while(p != NULL && (p->dato).genero != M){
        q = p;
        p = p->sig;
    }
    if(p == NULL){
        cout << "No hay libros de misterio" << endl;
        return;
    }
    if(q == NULL)
        pInicio = p->sig;
...
int main(){
    ...
    do{
        ...
        switch(opcion){
            ...
            break;
            case 5: cout<<"Buscando libros en espaniol..."<<endl;
			if (libroEspaniol())
				cout<<"SI hay libros en espaniol!"<<endl;
				else
				cout<<"NO hay libros en espaniol!"<<endl;
			break;
			case 6: cout<<"Buscando libros de Ciencia Ficcion..."<<endl;
			if(libroCF())
			cout<<"SI hay libros de Ciencia Ficcion!"<<endl;
			else
			cout<<"NO hay libros de Ciencia Ficcion!"<<endl;
			break;
			case 7: cout<<"Esta seguro que desea eliminar los libros de misterio? (s/n): ";
					cin>>p;
			if(p=='n' || p=='N')
			break;
			if(p=='s' || p=='S')
			cout<<"Eliminando libros de misterio..."<<endl;
			eliminarMisterio();
